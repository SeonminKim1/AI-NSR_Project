## ■ 기계학습 기반의 함수정보 구분 
### ■ 필요성 및 목표
- 현재 시중에 다양한 바이너리 분석 도구 존재(BAP, IDA Pro) 
- But.. 스트립(stripped)된 바이너리의 분석 정확도 낮음
- 스트립 바이너리(함수 위치 파악)에 대한 분석 어려움 -> 악성코드 분석 목적 기술필요
- 기계학습 기반의 스트립(stripped) 리눅스 바이너리 분석
- 함수 위치정보 '시작' 추출 기계학습 기반. 모델 연구    
<hr>
